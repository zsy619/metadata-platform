...
- [x] DefaultLayout（layouts/DefaultLayout.vue）
  - [x] 顶部导航栏（用户信息+退出+主题切换）
  - [x] 侧边菜单（多级菜单+折叠）
  - [x] 主内容区（router-view+keep-alive）
  - [x] 面包屑导航
  - [x] 响应式适配
- [x] Sidebar组件（layouts/components/Sidebar.vue）
  - [x] 菜单渲染（递归组件）
  - [x] 菜单高亮
  - [x] 菜单展开/折叠
  - [x] 图标显示
- [x] AppHeader组件（layouts/components/AppHeader.vue）
  - [x] Logo+系统名称
  - [x] 用户下拉菜单
  - [x] 全屏按钮
  - [x] 主题切换
- [x] Breadcrumb组件（layouts/components/Breadcrumb.vue）
  - [x] 自动生成面包屑
  - [x] 点击跳转
- [x] AuthLayout（登录页布局）

### 公共组件
- [x] DataTable组件（components/table/DataTable.vue）
  - [x] props配置化（columns/data/loading/pagination）
  - [x] 分页支持
  - [x] 排序支持
  - [x] 筛选支持
  - [x] 操作列插槽
  ...
  - [x] 加载状态
  - [x] 刷新功能
- [x] DynamicForm组件（components/form/DynamicForm.vue）
  - [x] JSON配置驱动
  - [x] 支持各种输入类型（input/select/date-picker/upload等）
  - [x] 表单验证
  - [x] 重置功能
  - [x] 只读/禁用模式
- [x] BaseDialog组件（components/dialog/BaseDialog.vue）
  - [x] 统一样式
  - [x] 多种尺寸
  - [x] 确认/取消按钮
  - [x] Loading状态
- [x] PageContainer组件（components/common/PageContainer.vue）
  ...
  - [x] 内容区
  - [x] 统一间距
- [x] EmptyState组件（components/common/EmptyState.vue）
  - [x] 图标+文字
  - [x] 操作按钮
- [x] Loading组件（components/common/Loading.vue）
  - [x] 全局Loading/局部Loading/骨架屏
- [x] SvgIcon组件（components/icon/SvgIcon.vue）

### API接口定义
- [x] Auth API（api/auth/index.ts）
  - [x] login/logout/getProfile/refresh/changePassword
- [x] DataSource API（api/data-source.ts）
  - [x] getList/create/update/delete/testConnection
  - [x] getTables/getViews/getTableStructure/previewData
- [x] Model API（api/model.ts）
  - [x] getList/create/update/delete
  - [x] buildFromTable/buildFromView
  - [x] getFields/addField/updateField/deleteField
- [x] API Management API（api/api.ts）
  - [x] getList/batchGenerate/update/delete/enable
- [x] QueryTemplate API（api/query-template.ts）
  - [x] getList/create/update/delete/setDefault
- [x] TypeScript类型定义（types/*.ts）
  ...
  - [x] API/APIParam
  - [x] QueryTemplate/QueryCondition
