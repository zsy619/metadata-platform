        ...
        <el-table v-loading="loading" :element-loading-text="loadingText" :data="filteredData" border stripe style="width: 100%; height: 100%;">
          <template #empty>
            <el-empty :description="searchQuery ? '未搜索到相关角色组' : '暂无角色组'">
              <el-button v-if="!searchQuery" type="primary" @click="handleCreate">新增角色组</el-button>
            </el-empty>
          </template>
...
import { createRoleGroup, deleteRoleGroup, getRoleGroups, updateRoleGroup } from '@/api/user'
...
const handleSubmit = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        formData.value.id ? await updateRoleGroup(formData.value.id, formData.value) : await createRoleGroup(formData.value)
