...
const loadData = async () => { loading.value = true; try { allData.value = await getUserGroups() } catch (error: any) { ElMessage.error(error.message) } finally { loading.value = false } }
const handleSearch = () => {}
const handleDebouncedSearch = () => {}
const handleCreate = () => { dialogTitle.value = '新增用户组'; formData.value = { state: 1, sort: 0 }; dialogVisible.value = true }
const handleEdit = (_row: any) => { dialogTitle.value = '编辑用户组'; formData.value = { ..._row }; dialogVisible.value = true }
const handleDelete = async (_row: any) => { try { await ElMessageBox.confirm('确定要删除该用户组吗？', '提示', { type: 'warning' }); await deleteUserGroup(_row.id); ElMessage.success('删除成功'); loadData() } catch (error: any) { if (error !== 'cancel') ElMessage.error(error.message) } }
const handleSubmit = async () => { if (!formRef.value) return; await formRef.value.validate(async (valid) => { if (valid) { submitLoading.value = true; try { formData.value.id ? await updateUserGroup(formData.value.id, formData.value) : await createUserGroup(formData.value); ElMessage.success(formData.value.id ? '更新成功' : '创建成功'); dialogVisible.value = false; loadData() } catch (error: any) { ElMessage.error(error.message) } finally { submitLoading.value = false } } }) }
onMounted(() => loadData())
</script>
