            ...
            <el-tabs v-model="activeTab">
                ...
                <el-tab-pane label="基本信息" name="basic" style="height: 300px;">
                    <el-form-item label="所属应用" prop="app_code">
                        <el-tree-select v-model="formData.app_code" :data="appTreeData" check-strictly :render-after-expand="false" placeholder="请选择所属应用（不选则不属于任何应用）" style="width: 100%" clearable @change="handleAppChange" />
                    </el-form-item>
                    <el-form-item label="上级菜单" prop="parent_id">
                        <el-tree-select v-model="formData.parent_id" :data="filteredMenuTreeData" check-strictly :render-after-expand="false" placeholder="请选择上级菜单（不选则为顶级菜单）" style="width: 100%" clearable />
                    </el-form-item>
                ...
                <el-tab-pane label="数据范围" name="dataScope" style="height: 300px;">
                    ...
                    <el-form-item label="数据权限" prop="data_scope" v-if="formData.data_range === '2'">
                        <el-tree-select v-model="selectedOrgIds" :data="orgTreeData" multiple check-strictly default-expand-all :render-after-expand="true" placeholder="请选择数据范围（组织）" clearable collapse-tags collapse-tags-tooltip style="width: 100%" :filter-method="filterOrg" />
                    </el-form-item>
                </el-tab-pane>
                <!-- 其他设置 -->
        ...
        if (valid) {
            ...
            try {
                const submitData = {
                    ...
                    menu_code: formData.value.menu_code,
                ...
                if (formData.value.id) {
                    await updateMenu(formData.value.id, submitData)
