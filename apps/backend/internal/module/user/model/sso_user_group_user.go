...
// SsoUserGroupUser 用户组关联用户表模型
type SsoUserGroupUser struct {
	ID        string    `json:"id" form:"id" gorm:"primary_key;type:varchar(64);column:id;comment:主键ID"`
	GroupID   string    `json:"group_id" form:"group_id" gorm:"type:varchar(64);default:'';uniqueIndex:idx_group_user;column:group_id;comment:组ID"`
	UserID    string    `json:"user_id" form:"user_id" gorm:"type:varchar(64);default:'';uniqueIndex:idx_group_user;column:user_id;comment:用户ID"`
...
// TableName 指定表名
func (SsoUserGroupUser) TableName() string {
	return "sso_user_group_user"
}
